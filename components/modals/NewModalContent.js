...
const NewModalContent = () => {
    ...
    return formPrompt <= 1 ? (<NewModalContentTextfieldForm_1.NewModalContentTextfieldForm handleSubmit={handleSubmit} onSubmitForm={submitSessionData} textInput={textInput} formPrompt={formPrompt} register={register}/>) : formPrompt <= 3 ? (<NewModalContentListForm_1.NewModalContentListForm handleSubmit={handleSubmit} addInputArray={addInputArray} removeInputArray={removeInputArray} addParameterList={addParameterList} submitInputArray={submitInputArray} setValue={setValue} formPrompt={formPrompt} sessionData={sessionData} textInput={textInput} inputArray={inputArray}/>) : (startNewSession && (<react_router_dom_1.Navigate to="/aogrid" state={{ from: "new", data: sessionData }} replace={true}/>));
};
