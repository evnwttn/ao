import { palette } from "./theme";

export const darkSideOfTheMoon = {
  id: "the dark side of the moon",
  author: "pink floyd",
  tracks: [
    {
      title: "speak to me",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey, comment: "" },
        { parameter: "guitar", colour: palette.aoGrey, comment: "this" },
        { parameter: "bass", colour: palette.aoYellow, comment: "" },
        { parameter: "drums", colour: palette.aoBlue, comment: "is" },
        { parameter: "piano", colour: palette.aoGrey, comment: "" },
        { parameter: "guest players", colour: palette.aoGrey, comment: "a" },
        { parameter: "guest vocals", colour: palette.aoGrey, comment: "" },
        { parameter: "fx", colour: palette.aoGrey, comment: "test" },
        { parameter: "mix", colour: "transparent", comment: "" },
        { parameter: "master", colour: "transparent", comment: "bruv" },
        { parameter: "done", colour: "transparent", comment: "" },
      ],
    },
    {
      title: "breathe (in the air)",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "on the run",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "time",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "the great gig in the sky",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "money",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "us and them",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "any colour you like",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "brain damage",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
    {
      title: "eclipse",
      parameters: [
        { parameter: "vocals", colour: palette.aoGrey },
        { parameter: "guitar", colour: palette.aoGrey },
        { parameter: "bass", colour: palette.aoYellow },
        { parameter: "drums", colour: palette.aoBlue },
        { parameter: "piano", colour: palette.aoGrey },
        { parameter: "guest players", colour: palette.aoGrey },
        { parameter: "guest vocals", colour: palette.aoGrey },
        { parameter: "fx", colour: palette.aoGrey },
        { parameter: "mix", colour: "transparent" },
        { parameter: "master", colour: "transparent" },
        { parameter: "done", colour: "transparent" },
      ],
    },
  ],
  parameters: [
    "title",
    "vocals",
    "guitar",
    "bass",
    "drums",
    "piano",
    "guest players",
    "guest vocals",
    "fx",
    "mix",
    "master",
    "done",
  ],
};
